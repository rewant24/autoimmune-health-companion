...
> **Drafted:** 2026-07-12 · **Owner:** orchestrator (Claude Code main) · **Status:** DRAFT — Track A + Track B fix pass (3 lenses, same session) applied; Rewant's OQ answers (OQ-1…OQ-15, see feature MD) + copy sign-off pending
...
- W2-2: every range read pushes bounds into `withIndex`; documented exemption classes only (currently one: the first-check-in read in `getReportWindow`, annotated `// W2-2 exemption`, short-circuited) — any new class must be annotated the same way AND named in the PR description. Marker name discovery is NOT an exemption: it rides a bounded `bloodWork.by_user_date` parent read (US-6.A.2).
- Cancelled events: never on page-1 surfaces or completed-counts; in appendix/strip as greyed line items with the *"Cancelled"* pill. Soft-deleted excluded everywhere. **Data-model reality (OQ-13):** the `status`/`linkedVisitId` fields are pre-flight prerequisite 3 (F05-patch PR); if it hasn't landed, C1 builds degraded — every visit renders completed, the honest-count line is omitted (k = 0 rule), status-dependent tests ship skipped-with-reason.
...
- Copy — two tiers, do not conflate: **Locked (scoping-verbatim or shipped vocabulary):** window line *"From your last visit on {date} to today, {date}"* / *"The last {N} days"* · granularity *"Daily / Weekly / Monthly"* · section *"Talking Points for Your Visit"* · pills *"APPOINTMENT" / "BLOOD WORK" / "Cancelled"*. **Proposed (draft-new — Rewant's Task 0 copy sign-off gates dispatch):** tab *"Report"* · heading *"Doctor Report"* · narrative templates per US-6.B.2 · patterns fallback *"This section fills in once Saha's pattern view is ready — your numbers are already being kept."* · empty-state strings (US-6.C.2) · popover strings + honest-count phrasing (US-6.C.3) · metric labels (US-6.B.3). Language guardrail (locked, product-level): "support-system", never "caregiver"/"squad"; first-person Saha; witness-don't-prescribe.
...
**Steps:**
- [ ] Confirm Rewant answered: OQ-2 fork state (read F03 OQ-2's resolution — determines the adapter's initial state), OQ-3 (talking-points source), OQ-4 (headline set), OQ-6 (snapshot-fix ownership), OQ-9 (no-anchor default), OQ-10 (buckets), OQ-12 (lab read source), OQ-13 (F05 status patch: merged or build-degraded), OQ-15 (F03 gate inheritance — needed only if F03 is live at dispatch) — **stop if not**; feature-MD defaults are proposals, not decisions. (OQ-1/5/7/8/11/14 are C2/C3 questions — they don't gate this cycle.)
- [ ] **Copy sign-off:** Rewant has signed off (or consciously deferred) every **proposed** string — tab + heading, narrative templates, patterns fallback line, empty-state copy, popover strings + honest-count phrasing, metric labels. Proposed ≠ locked; anything unsigned ships flagged in the PR description, never silently promoted to a lock.
...
- [ ] **Prerequisite 3 — doctor-visit status patch (OQ-13):** verify the F05-patch PR (`status` + `linkedVisitId` + cancel/reschedule mutations + capture UI) is merged, or apply the degrade path (all visits completed; honest-count omitted; status-dependent tests skipped-with-reason). Either way, note the state in build-log — the build agents must know which world they're in.
- [ ] **Prerequisite 4 — F03 contract source:** PR #39 merged, or its `03-patterns.md` attached verbatim to the 6.B dispatch prompt — the normative F03 inputs (series-types contract fields, R2-1/R2-2 rules, the OQ-8 IST lock) live on `docs/f03-chunking`, not `main`.
...
Stories: US-6.A.1 `getReportWindow` (last live completed visit ≤ today via bounded desc read — status absent pre-prerequisite-3 reads as completed, OQ-13; `defaultOnDemandFrom` = anchor + 1 IST day; `onDemandWindowEmpty` flag; `upcomingVisits` asc capped at 5; `historyDays` clamped ≥ 0; annotated W2-2-exempt first-check-in read, short-circuited, skips soft-deleted; extracted handlers, structural mock-ctx per `convex/checkIns.ts` house pattern) · US-6.A.2 `getReportEvents` + `getMarkerTrends` (all reads index-bounded; visit status when the field exists, else completed; deactivated-med labels resolve; intake snapshot-preferred with `snapshotMissing` join fallback; **marker name discovery via bounded `bloodWork.by_user_date` parent read + `canonicalMarkerName()` + case-insensitive grouping — never an unbounded scan**; per-name trends off `by_user_name_date`; `direction90d` trailing-90d anchored to `toDate`, null under 2 points OR on mixed units) · US-6.A.3 devSeed **end-state** extension (35d, 2 visits — plus 1 cancelled only if prerequisite 3 landed, 2 blood works with CRP/ESR trend, 20d intake; wipe: checkIns by `providerUsed:"seed"`, meds by `[seed] ` name prefix, doses AND intake events via `medicationId` resolving to a `[seed] ` med, visits by `[seed] ` doctorName prefix, bloodWork by reserved `seed-bloodwork-n` clientRequestIds via exact `by_user_client_request` lookups; idempotent; fork-independent — assert state, not delta).

Test approach: mock-ctx handler tests (house pattern) + fixture builders. Required cases in the feature MD acceptance (null anchors, visit-today edge → `onDemandWindowEmpty`, soft-deleted latest visit, soft-deleted/future-dated first check-in, canonical + case-variant name merge, mixed-unit null arrow, historical-`toDate` arrow anchor, soft-deleted parent excludes projections, zero-marker row, pre/post-snapshot intake rows, seed idempotency + full wipe incl. intake). Status-dependent cases (only-cancelled history, rescheduled pair, cancelled + completed same window) ship skipped-with-reason unless prerequisite 3 landed (OQ-13).
...
Stories: US-6.B.1 window + bucket math (`defaultGranularity` bands 14/90, input clamped ≥ 0; IST Monday-start calendar buckets per OQ-10; `bucketize` accepts absent metric series → event-only buckets; intake aggregate = event **count**, never taken/expected — `medications.frequency` is free-form (ADR-030); mood modal tie → most recent day; `windowLabel` locked strings; `clampWindow` 7d floor for **auto windows only** — anchored 1–6-day windows pass through; pure — caller supplies `today`; mean/mode/percent bucket aggregation, no fake zeros) · US-6.B.2 narrative rules engine (template catalog only — granularity-independent by construction; flare bullets require the fork-gated `flareSpans` input, dose/lab/visit bullets always; `NARRATIVE_DOSE_LOOKBACK_DAYS = 14` exported; banned-causation-words grep test) + talking-points selector (priority flares > worst pain > dose-change > declined > recent; `TALKING_POINTS_MAX = 8`; 140-char word-boundary excerpts; tap-only/empty transcripts skipped) · US-6.B.3 headline assembly (`core` fork-gated group + `alwaysOn` group; dose-change count always from F06's own arithmetic even with `patternsSummary` present — single source; arrows up/down/flat/null, label omits the arrow on null; both-fork + runtime-empty unit tests) + the adapter (stub types pinned against the F03 contract mirror by a compile/test guard).
...
Stories: US-6.C.1 route + tab fork (compose header/strip/chart-or-fallback/events/narrative/talking-points; required-query (window/events) failure → F10-shaped connection-error template + retry; marker-trends failure alone → lab section absent, report renders; JourneyTabs: extend to *Memory · Patterns · Report* if the file exists, create as *Memory · Report* if not — both branches tested, inactive branch skipped-with-reason) · US-6.C.2 ADR-015 empty states (fresh + empty-window variants; **data-only predicate — zero check-ins AND zero events, ever vs in-window; `patternsSummary` never in the condition**; reactive after check-in save; proposed copy per feature MD flagged for Rewant) · US-6.C.3 events strip + granularity (44pt hit targets with slop, same-day clustering + count badge, cancelled greying + pill + honest-count line **only post-prerequisite-3 — pre-patch all visits render completed, line omitted (OQ-13)**, popovers; toggle re-buckets the bucketed metric aggregates only — narrative + headline strip are granularity-independent; session-persisted selection, scroll position preserved).
...
- [ ] Grep guards: no `.collect()` without index bounds in `convex/report.ts` except annotated `// W2-2 exemption` reads (documented classes per the locked-decisions block — currently the first-check-in read; any new annotated class must be named in the PR description, and marker name-discovery must NOT appear here — it rides the bounded parent read) · no mutation exports in `report.ts` · no `api.patterns` / `components/patterns` / `lib/patterns` imports outside `lib/report/patterns-summary.ts` · no `localStorage.getItem('saha.testUser` outside the auth seam · no causation words in the narrative catalog · no PDF-library import anywhere (C2 scope).
...
- Copy verbatim against the two-tier list in the locked-decisions block: scoping-verbatim locks (window lines, granularity labels, talking-points title, pills) AND the Task-0-signed proposed strings (tab/heading, narrative templates, fallback line, empty-state/popover/metric-label copy); "support-system" guardrail; first-person Saha in user-facing lines; **no causation language anywhere** (product-level no-diagnosis lock).
...
- Read-only invariant: `convex/report.ts` exports queries only; no schema diff; devSeed wipe semantics per US-6.A.3 (all five marker conventions, incl. intake-via-medicationId).
...
### Review-3 (edge cases)
- Windows/anchors: zero visits ever · only-upcoming visits · visit dated today (`onDemandWindowEmpty` guard) · anchor visit soft-deleted after view opened (reactive recompute) · future-dated first check-in (`historyDays` clamp). Status-dependent (only if prerequisite 3 landed — OQ-13): only-cancelled history · rescheduled pair.
- Data: zero check-ins (fresh state) · data-but-empty-window · **window with check-ins but zero events (report renders — talking points + buckets, never the empty state; the data-only predicate's load-bearing case)** · all-metrics-declined window · window with events but no check-ins (strip + narrative render, talking points empty) · deactivated-med dose change · canonical + case-variant names merging into one lab trend · marker trend with 1 point or mixed units (null arrow, arrow omitted in the label) · pre-snapshot intake rows (`snapshotMissing` rendering) · 365-day monthly window (bucket count, perf) · window shorter than one bucket · month/year boundary buckets · 3-day anchored window not stretched by the floor.
...
- UI: 320px · 44pt hit targets incl. strip slop · same-day event clustering badge · cancelled-vs-completed AA distinguishability (post-prerequisite-3 only) · `prefers-reduced-motion` · aria labels · popover dismissal.
...
- [ ] **Manual smoke on the preview URL (6 steps):** ① fresh user → Report tab shows the fresh ADR-015 template ② seeded user → header window line correct against the seeded anchor visit ③ headline strip: always-on numbers correct; patterns block matches the fork state (chart or fallback copy — never blank) ④ events strip: completed visits + cluster badge render; if prerequisite 3 landed — cancelled greying + pill + honest-count line correct; if not — every visit renders completed and NO honest-count line appears (its absence IS the pass condition, OQ-13) ⑤ granularity toggle Daily→Weekly visibly re-buckets the metric aggregates (bucket count + labels change); narrative bullets and the headline strip stay identical — a narrative that changes with the toggle is a FAIL ⑥ talking points show dated verbatim excerpts, flare day ranked first.
...
- Anything requiring a `convex/schema.ts` edit, a new mutation, or a new index → stop; that breaks the F06 C1 read-only invariant and needs Rewant (the intake-snapshot fix AND the visit-status patch belong to their own pre-flight PRs — OQ-6, OQ-13 — never to this branch).
