...
- **Acceptance:**
  ...
  - **UX copy:** success heading: "Got it. See you tomorrow, Saha's here." Error: "Couldn't save just now. Try again?" Save-later button: "Keep this for later".
...
- **Acceptance:**
  ...
  - **UX copy:** "You're seeing the last 30 days. Saha Companion keeps your full history. — See plans".
...
- `docs/CLAUDE.md` — rewritten top section so every new session auto-loads canonical context. Exact header to prepend:
  ...
  ```markdown
  # Saha — Autoimmune Health Companion
...
7. **Session-start context rule:** every new session about Saha auto-loads the 5 canonical docs (scoping, build-plan, system-map, product-taxonomy, tech-stack) + named feature MD if relevant. Enforced via `docs/CLAUDE.md` header (project-level) and a memory file pointer (cross-tab).
